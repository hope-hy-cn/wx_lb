...
<template> 
  <div class="new-detail"  :style="'padding-top:'+headHeight+'rpx'"> 
    <div class="head-back"  :style="'height:'+headHeight+'rpx;padding-top:'+headPadding+'rpx;'" v-if="!headType">  
      <div class="icon-box" @click="goBack">
        <image src="/static/image/home-icon.png" class="home-cion"></image>
      </div> 
    </div> 
    <head-back title="帖子详情"  @goBack="goBack" :iconShow="true" v-if="headType=='detail'"></head-back> 
    <div class="content">
      <div class="newsList" v-if="!(JSON.stringify(newData) == '{}')">
      ...
      <div class="commentCont">
        ...
        <div class="listCont" v-if="commendList.length>0">
          <div class="listItem" v-for="(item,index) in commendList" :key="index">
            ...
            <div class="rightCont">
              ...
              <!-- <div class="topInfo" @click="goToUserDefault(item)">
                <div class="userName">{{item.nickname?item.nickname:"匿名用户"}}</div>
                <div class="dispatchTime">{{item.create_time}}</div>
              </div>
        ...
        <div class="noData"  v-if="commendList.length==0">
...
export default { 
  onLoad(opt){       
    ...
    this.commendList=[];
    if(opt){ 
      opt.headType?vm.headType=opt.headType:vm.headType='';
      this.commentListInfo.article_id=opt.artId;
      this.getArtDetail(opt.artId);
      this.clearListData();
  ...
  methods: { 
    ...
    //跳转帖子界面  
    goBack(){   
      if(this.headType=='detail'){
        wx.navigateBack({ delta: 1 }); 
      }else{
        let url = `./../../common/news/main`;
        wx.switchTab({url});
      }
    ...
    getArtDetail(val){
      let vm = this;
      api.focusArtDetail({article_id:val}).then(res=>{   
    ...
    getCommentList(fresh){
      ...
      return api.commentList(vm.commentListInfo).then( res=>{ 
        if(res.data.code==300000){
          let resData=res.data.data.data;
          vm.commentListInfo.total_page=res.data.data.info.total_page;
          vm.commenedNum=res.data.data.info.total; 
          if(resData){
            resData.forEach((item,index)=>{
              item.create_at?item.create_time=vm.getDateDiff(item.create_at):item.create_time='';
            }) 
            vm.commendList.push(...resData);
          } 
        }
      }).catch(err=>{
        console.log(err);
    ...
    dispatchComment(){
      if(this.currentComment.trim()==''){
        ...
        return api.dispatchComment({
          article_id:parseInt(this.commentListInfo.article_id),
  ...
  onShareAppMessage: function(res) {  
    ...
    let imgLink = this.newData.pics?this.newData.pics[0]:this.newData.share_img;
