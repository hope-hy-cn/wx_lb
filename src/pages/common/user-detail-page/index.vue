        ...
        <div class="homepage">
          ...
          <div style="margin-top:24rpx;">
            <block v-for="(item,index) in artData" :key="index"  v-if="artData.length>0">
              <news-list :dataItem="item"  @leaveWord="leaveWord" more="true" @deleteItem="deleteArt" @isLogin="checkIsLogin" @jumpTopic="jumpTopic(item)" @share="shareItem"> </news-list>
            </block>
            <div style="height:102rpx;width:100%;" v-if="artData.length>0">
...
export default { 
  onLoad(opt){       
    ...
    if(opt.ruuid){ 
      vm.ruuid=opt.ruuid;
      vm.getMyCaInfo(opt.ruuid);
      vm.getUserpageArt(opt.ruuid);
      vm.getUserDetail(opt.ruuid); 
      // vm.getUserpageArt('52769075-b2c1-4661-81d2-f4035348c8f4');
    }else{
  ...
  methods: {   
    ...
    scrolltolower(){
      //在个人主页以及下滑的时候
      if(this.endClientY - this.startClientY<0){
        if (this.isLoadEnd) {
          return;
        } else {
          if (!this.isLoad) {
    ...
    goBack(){  
      let timer=setTimeout(()=>{
        app.aldstat.sendEvent('detail:返回');
        wx.navigateBack({ delta: 1 });  
        clearTimeout(timer);
      },300);
    },   
          ...
          wx.showModal({
            ...
            success(res) {
              if (res.confirm) {
                me.isLike=false;
                me.likeIcon=false;
                api.cancelLike({ruuid:me.ruuid}).then(
                  ()=>{
