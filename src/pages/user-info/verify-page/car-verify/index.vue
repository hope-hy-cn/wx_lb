<template>
  <div class="edit-info" :style="'padding-top:'+headHeight+'rpx'">
    <head-back title="车辆认证"  @goBack="goBack" ></head-back>
    <div class="content">
      <div class="group-title">信息仅供此次认证使用，将严格保密</div> 
      <div class="main">
        <div class="main-box">
          <div class="car-img" @click="addCode(1)">
            <image  mode="scaleToFill"  :src="imgOnePage?imgOnePage:'/static/image/car-mian.png'" class="img"  />
          </div>
          <div class="title text-center">拍照行驶证<span style="color:#4A90E2;">主页</span></div> 
        </div> 
        <div class="main-box">
          <div class="car-img" @click="addCode(2)">
            <image  mode="scaleToFill"  :src="imgtwoPage?imgtwoPage:'/static/image/car-vice.png'" class="img"  />
          </div>
          <div class="title text-center">拍照行驶证<span style="color:#4A90E2;">副页</span></div> 
        </div> 
      </div> 
      ...
      if(vm.clickPoint){
        wx.uploadFile({
          ...
          success: function (res) {   
            ...
            if(vm.postIndex==2){
              vm.clickPoint=false;
              api.updateCaLook({car_front_pic:vm.linkList[0],car_reverse_pic:vm.linkList[1]}).then(msg=>{
                vm.clickPoint=true;
