<template>
    <div class="recommendCont">
        ...
        <div id="innerScrollCont" :style="'height:'+innerHeight">
            ...
            >
                <div class="tagItem" v-for="(it,idx) in listPage.tagArr" :key="idx" @click="jumpToTagList(it)">
                    <img src="/static/image/hash-hover.png" alt="">{{it.topic}}
                </div>
            </scroll-view>
            ...
            <news-list v-for="(it,idx) in listPage.listArr" :key="idx" :dataItem='it' @jumpDetail="jumpDetail" @leaveWord="leaveWord" @jumpTopic="jumpToTagList" @isLogin="isLogin"  @share="shareItem"></news-list>
...
export default {
    ...
    onLoad(){
        ...
        this[this.filterGetListType(1)](this.listPage.pageInfo);
    },
    props:['currentPage'],
    watch:{
        currentPage(val){
    ...
    methods:{
        ...
        //获取推荐的列表
        getRecommendList(body,num){ 
            return api.recommendList(body).then(
                ({data})=>{ 
                    if(data.code==300000&&data.data.data){
                        if(this.reFresh){
                            ...
                            let timer=setTimeout(()=>{
                                ...
                                this.listPage.listArr=data.data.data;
                                clearTimeout(timer);
                            },100);
                        }else{
                            let timer=setTimeout(()=>{
                                this.innerHeight='auto';
                                this.listPage.listArr.push(...data.data.data);
                                clearTimeout(timer);
                            },100);
                        };
                        this.loadState(0);
                        data.data.topics?this.listPage.tagArr=data.data.topics:this.listPage.tagArr=[];
                        this.listPage.pageInfo.total=data.data.info.total_page;
                ...
                if(!this.isLoad){
                    ...
                    if(currentPageInfo.page>=currentPageInfo.total){
                        ...
                        let time_=setTimeout(()=>{
                            this[this.filterGetListType(1)](currentPageInfo);
                            clearTimeout(time_);
                        },50)
                    }
                }
