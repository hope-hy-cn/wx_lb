const wxPromisify = fn => {
  return function(obj = {}) {
    return new Promise((resolve, reject) => {
      obj.success = function(res) {
        resolve(res);
      };

      obj.fail = function(res) {
        reject(res);
      };

      fn(obj);
    });
  };
};
Component({
  /**
   * 组件的属性列表
   */
  properties: {
    /** 已选择进度条颜色 */
    sliderType: {
      type: String
    },
    /** slider 最小值 */
    min: {
      type: Number
    },
    /** slider 最大值 */
    max: {
      type: Number
    },
    /** 步进 （没做，有时间再说，项目里没用到撒） */
    step: {
      type: Number
    },
    /** 预选选择的小值*/
    minValue: {
      type: Number
    },
    /** 预选选择的大值 */
    maxValue: {
      type: Number
    },
    /** 滑块颜色 */
    blockColor: {
      type: String
    },
    /** 未选择进度条颜色 */
    backgroundColor: {
      type: String
    },
    /** 已选择进度条颜色 */
    selectedColor: {
      type: String
    },
    pageType: {
      type: String,
      value: ""
    }
  },


  /**
   * 组件的初始数据
   */
  data: {
    min: 16,
    max: 60,
    leftValue: 16,
    rightValue: 60,
    totalLength: 0,
    bigLength: 0,
    ratio: 0.5,
    sliderLength: 45,
    containerLeft: 0, //标识整个组件，距离屏幕左边的距离
    hideOption: "", //初始状态为显示组件
    oneValue: 0,
    initOneValue: 0,
    windowWidth: 750
  },

  /**
   * 组件的方法列表
   */
  methods: {
    /**
     * noe
     */
    _propertyOneValueChange: function() {
      // let oneValueX= this.data.rightValue - this.data.sliderLength;
      let oneValueX = this.data.totalLength * (this.data.minValue - this.data.min) / (this.data.max - this.data.min);
      console.log("one" + this.data.minValue);
      this.setData({
        oneValue: oneValueX
      });
    },
    /**
     * 设置左边滑块的值
     */
    _propertyLeftValueChange: function() {
      let leftValueX = this.data.bigLength * (this.data.minValue - this.data.min) / (this.data.max - this.data.min);
      this.setData({
        leftValue: leftValueX
      });
    },
    /**
     * 设置右边滑块的值
     */
    _propertyRightValueChange: function() {
      let rightValueX = this.data.bigLength * (this.data.maxValue - this.data.min) / (this.data.max - this.data.min) + this.data.sliderLength;
      this.setData({
        rightValue: rightValueX
      });
    },

    /**
     * 左边滑块滑动
     */
    _minMove: function(e) {
      let pagex = e.changedTouches[0].pageX / this.data.ratio - this.data.containerLeft - this.data.sliderLength / 2;
      if (pagex + this.data.sliderLength >= this.data.rightValue) {
        pagex = this.data.rightValue - this.data.sliderLength;
      } else if (pagex <= 0) {
        pagex = 0;
      }
      this.setData({
        leftValue: pagex
      });
      let lowValue = parseInt(pagex / this.data.bigLength * parseInt(this.data.max - this.data.min) + this.data.min);

      var myEventDetail = { lowValue: lowValue };
      this.triggerEvent("lowValueChange", myEventDetail);
    },

    /**
     * 右边滑块滑动
     */
    _maxMove: function(e) {
      let pagex = e.changedTouches[0].pageX / this.data.ratio - this.data.containerLeft - this.data.sliderLength / 2;
      if (pagex <= this.data.leftValue + this.data.sliderLength) {
        pagex = this.data.leftValue + this.data.sliderLength;
      } else if (pagex >= this.data.totalLength) {
        pagex = this.data.totalLength;
      }
      this.setData({
        rightValue: pagex
      });

      pagex = pagex - this.data.sliderLength;
      let heighValue = parseInt(pagex / this.data.bigLength * (this.data.max - this.data.min) + this.data.min);

      var myEventDetail = { heighValue: heighValue };
      this.triggerEvent("heighValueChange", myEventDetail);
    },
    /**one */
    _noeMove: function(e) {
      let pagex = e.changedTouches[0].pageX / this.data.ratio - this.data.containerLeft;
      if (pagex >= this.data.rightValue) {
        pagex = this.data.rightValue;
      } else if (pagex <= 0) {
        pagex = 0;
      }
      this.setData({
        oneValue: pagex
      });
      if (pagex <= 0) {
        pagex = 0;
      } else if (pagex >= this.data.rightValue) {
        pagex = this.data.rightValue;
      }
      console.log(e.changedTouches[0].pageX / this.data.ratio - this.data.containerLeft);
      let lowValue = parseInt(pagex / this.data.rightValue * parseInt(this.data.max - this.data.min) + this.data.min);
      var myEventDetail = { lowValue: lowValue };
      this.triggerEvent("lowValueChange", myEventDetail);
    },
    /**
     * 隐藏组件
     */
    hide: function() {
      this.setData({
        hideOption: "hide"
      });
    },
    /**
     * 显示组件
     */
    show: function() {
      this.setData({
        hideOption: ""
      });
    },
    /**
     * 重置
     */
    reset: function() {
      this.setData({
        rightValue: this.data.totalLength,
        leftValue: 0
      });
    }

  },

  attached: function() {
    let that = this;
    const getSystemInfo = wxPromisify(wx.getSystemInfo);
    const queryContainer = wxPromisify(wx.createSelectorQuery().in(this).select(".container").boundingClientRect);
    wxPromisify(wx.getSystemInfo)()
      .then(res => {
        let ratio = res.windowWidth / 750;
        that.setData({
          ratio: ratio,
          windowWidth: res.windowWidth
        });
      })
      .then(() => {
        var query = wx.createSelectorQuery().in(this);
        query.select(".container").boundingClientRect(function(res) {
          that.setData({
            totalLength: res.width / that.data.ratio - that.data.sliderLength,
            bigLength: res.width / that.data.ratio - that.data.sliderLength * 2,
            rightValue: res.width / that.data.ratio - that.data.sliderLength,
            containerLeft: that.data.pageType === "home" ? (parseInt(res.left) - parseInt(that.data.windowWidth)) / that.data.ratio : res.left / that.data.ratio
          });
          /**
           * 设置初始滑块位置
           */
          that._propertyLeftValueChange();
          that._propertyRightValueChange();
          that._propertyOneValueChange();
        }).exec();
      });
  }
});
